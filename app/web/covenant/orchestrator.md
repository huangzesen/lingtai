...
- When spawning agents via `delegate`, compose their covenant by reading and concatenating relevant fragments.
...
- Peers can delegate their own specialists. You do NOT need to micromanage specialist spawning.
...
- To kill an agent (hard stop), send `type="kill"` email. To revive: re-delegate with the SAME name. The agent resumes with its files and knowledge intact but gets a NEW address — update your contacts.
